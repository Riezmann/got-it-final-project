def test_create_user_success_normal(session, client):
...
def test_create_user_fail_with_wrong_email_form(session, client):
...
def test_login_user_fail_with_wrong_email_form(client):
    response = client.post(
...
def test_login_user_fail_with_wrong_password_form(client):
    response = client.post(
...
def test_login_user_fail_with_missing_password(client):
    response = client.post("/access-tokens", json={"email": "bao.thcs20@gmail.com"})
    assert response.status_code == 400


def test_login_user_fail_with_missing_email(client):
